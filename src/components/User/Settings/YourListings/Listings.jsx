...
export default function YourListings(){
    ...
    return <Box minHeight={"100vh"} minWidth={"60vw"} textAlign={"left"}>
        <Box><Typography variant="h4" color={lb} fontWeight={700}> Your Listings</Typography></Box>
        <br />
        {pendingItems.reverse().map((el,i)=>{
            return <ListingCards key={i} name={el.name} status={el.status} price={el.price} img={el.Img} />
        })}
    </Box>
}